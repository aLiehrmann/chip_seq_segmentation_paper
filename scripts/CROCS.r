...
solver <- function(counts, weights, lambda, loss_type, graph, disp = 0) {
  res_seg <- gfpop(mygraph = list_of_graphs[[graph]](lambda), data = counts, weights = weights, type = loss_type)
  nb_peaks <- number_of_peaks(res_seg$parameters)
  theta <- rep(res_seg$parameters, times = diff(c(0, res_seg$changepoints)))
  loss <- list_of_losses[[loss_type]](counts, theta, disp, weights)
  list(
    ...
    lambda = lambda,
    parameters = res_seg$parameters,
    changepoints = res_seg$changepoints
  )
}


sequential_search <- function(counts, weights, target, loss_type, graph, disp = 0) {
  upper_bound_model <- solver(counts, weights, 0, loss_type, graph, disp)
  lower_bound_model <- solver(counts, weights, Inf, loss_type, graph, disp)
  while (target != lower_bound_model$nb_peaks & target != upper_bound_model$nb_peaks) {
    ...
    new_candidate_model <- solver(counts, weights, new_candidate_lambda, loss_type, graph, disp)
...
CROPS <- function(counts, weights, lower_bound_lambda, upper_bound_lambda, loss_type, graph, disp = 0) {
  list_of_models <- list()
  last_index <- 1
  list_of_models[[last_index]] <- solver(counts, weights, lower_bound_lambda, loss_type, graph, disp)
  last_index <- last_index + 1
  list_of_models[[last_index]] <- solver(counts, weights, upper_bound_lambda, loss_type, graph, disp)
  q <- queue()
  pushback(q, c(1, 2))
  while (length(q) != 0) {
    d <- pop(q)
...
CROCS <- function(counts, weights, lower_bound_peak, upper_bound_peak, loss_type, graph, disp = 0) {
  if (lower_bound_peak > 0) {
    lower_bound_peak <- lower_bound_peak - 1
  }
  upper_bound_peak <- upper_bound_peak + 1
  lower_bound_lambda <- sequential_search(counts, weights, upper_bound_peak, loss_type, graph, disp)$lambda
  upper_bound_lambda <- sequential_search(counts, weights, lower_bound_peak, loss_type, graph, disp)$lambda
  CROPS(counts, weights, lower_bound_lambda, upper_bound_lambda, loss_type, graph, disp)
}
...
res <- future_map(unique(dt.counts$sample.id), function(x) {
  ...
  CROCS(counts, weights, 1, 9, "poisson", "PDPA")
}, .options = furrr_options(seed = 2020))
names(res) <- unique(dt.counts$sample.id)
map(res, function(y){sort(map_dbl(y, ~ .x$nb_peaks))})