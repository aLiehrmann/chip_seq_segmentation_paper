...
res_cv_all_linear_vs_constant_lambda <- res_cv_all_linear_vs_constant_lambda[grep(res_cv_all_2$model, pattern="linear_phi",invert=T),]
...
g <- ggplot(aes(y=as.factor(lab2), x=diff.accuracy), data=res_cv_all_max_jump_vs_other)+
  ...
  xlab("accuracy(target model)% - accuracy(reference model)%")+
  ylab("target model")+
  theme(
    strip.background = element_rect(fill="grey95"), 
    strip.text.y = element_text(angle = 0),
